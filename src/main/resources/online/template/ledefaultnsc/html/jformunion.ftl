...
<head>
 ...
 $(document).ready(function(){
	...
	$("#xiabu").attr({"src":"NSC选型明细.do?list&id=nsc泵性能参数","状态":"查询"});
  ...
  function uploadFile(data){
	...
	<#list subtablelist as sub >
	    <#if field['${sub}']?exists >
		    ...
		    <#else>
		    <iframe class="J_iframe" name="iframe3" width="100%" height="100%"  src="NSC选型明细.do?list&id=${sub}" frameborder="0"  id="xiabu" seamless 状态="初始化" > </iframe>
		    </#if>
		</#if>
	</#list>
