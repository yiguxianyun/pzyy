...
@Controller
@RequestMapping("/列表")
public class 列表控制器 extends BaseController{
	@Autowired
	...
	private static Logger log = Logger.getLogger(列表控制器.class);
	...
	public void datagrid(String configId,String page,String field,String rows,String sort,String order, HttpServletRequest request,
		...
		//kk查询之前过滤
		if (configId.equals("待办任务视图")){
			通用控制器.过滤待办任务(request,username);			
		}else if(configId.equals("已结束流程视图")&&!username.equals("admin")){
			通用控制器.过滤已结束流程(request,username);	
		}
		...
		//kk查询后过滤
        List<String> 角色列表 = Arrays.asList(new String[] {"管理员","公司领导"});
        List<String> 表单列表 = Arrays.asList(new String[] {"运行中流程视图","已结束流程视图"});        
		if (  表单列表.contains(configId)&&!角色列表.contains(角色)  ){
			通用控制器.过滤本人或部门(request,result,username,角色);
		}
...
//						}
	...
	public AjaxJson delBatch(String configId,String ids,
		...
		String message ="删除成功";		
		try {
			String[] id数组 = ids.split(",");
			for(String id:id数组){				
				String 流程id = 通用控制器.查流程id(configId,id,request);
				if(StringUtils.isNotEmpty(流程id)){
					String 角色=通用控制器.查用户("角色",request);
					Map 待办任务=通用控制器.查流程(流程id, request);
					String 步骤 = (String)待办任务.get("步骤");
					if(StringUtils.isEmpty(步骤)){
						步骤="";
					}
					if(!角色.equals("管理员")&&(!步骤.equals("tianxie")||!步骤.equals("1"))){
						message = "不能删除！！！流程已经不是第一步！";
						break;
					}
				}
			}
			if((message.indexOf("不能删除")==-1)){
				cgTableService.deleteBatch(table, id数组);
			}			
		} catch (Exception e) {
