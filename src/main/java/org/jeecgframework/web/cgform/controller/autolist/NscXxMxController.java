...
import org.jeecgframework.web.cgform.service.autolist.NscXxMxServiceI;
 ...
 */
@Controller
@RequestMapping("/NSC选型明细")
public class NscXxMxController extends BaseController{
	@Autowired
	private ConfigServiceI configService;
	@Autowired 
//	@Resource(name="NscXxMxService") 
	private NscXxMxServiceI NscXxMxService;
	@Autowired
	...
	public void list(String id, HttpServletRequest request,
		...
		String kk="online/template/ledefault/htmlkk/nsc_xx_mx.ftl";
	...
	public void datagrid(String configId,String page,String field,String rows,String sort,String order, HttpServletRequest request,
		...
		String 状态 = request.getParameter("状态");
		String qs = request.getParameter("qs");
		String qx = request.getParameter("qx");
		String hs = request.getParameter("hs");
		String hx = request.getParameter("hx");
		String js = request.getParameter("js");
		js=js.isEmpty()?"":"and 电机极数 ="+js;
		if(状态.equals("编辑")){
			if(StringUtils.isBlank(wj)){
				params = "1=2";
			}else{params = "wj='"+wj+"'";}
		}else{
			params=MessageFormat.format("流量h >= {0} AND 流量h <= {1} and 扬程 >= {2} AND 扬程 <= {3} {4}",
					qx , qs ,hx,hs,js);
		}
...
//		}
		...
		if(isTree && treeId !=null) {
			//防止下级数据太大，最大只取500条
//			result=NscXxMxService.querySingle(table, field.toString(), params,sort,order, 1, 500);
		}else {
			result=NscXxMxService.querySingle(table, field, params,sort,order, p,r );
		}
...
//		return result;
		//treeform 处理是否有下级菜单
//		if(isTree) {
//			NscXxMxService.treeFromResultHandle(table, parentIdFieldName, parentIdFieldType,
//					result);
//		}
...
//		Long size = NscXxMxService.getQuerySingleSize(table, field, params);
		Long size=(long) 9;
...
//		NscXxMxService.delete(table, id);
		String message = "删除成功";
	...
	public AjaxJson delBatch(String configId,String ids,
		...
		try {
			String[] id = ids.split(",");
//			NscXxMxService.deleteBatch(table, id);
		} catch (Exception e) {
